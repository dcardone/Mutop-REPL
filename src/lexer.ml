...
let rec trim_quotes input balanced = if String.length input = 0 && balanced then input else if balanced then 
    (if String.get input 0 = '\"' then trim_quotes (String.sub input 1 (String.length input - 1)) false else input) else
    if String.length input = 0 then raise (InvalidInputException "Improper quotes") else 
    if String.get input ((String.length input) - 1) = '\"' then trim_quotes (String.sub input 0 (String.length input - 1)) true else
    input

let check_if_ID input snippet = if Str.string_match idofstringRegex input 0 then let matched = Str.matched_string input in
    if String.length matched > String.length snippet then matched else snippet
    else snippet

let rec tokenize_helper input acc = let length = String.length input in
    if length = 0 then acc else
    if Str.string_match whiteSpaceRegex input 0 then tokenize_helper (Str.string_after input 1) acc else
    ...
    if Str.string_match intofintRegex input 0 then let matched = Str.matched_string input in
        if String.contains matched '(' then (tokenize_helper (Str.string_after input (String.length matched)) (acc @ [Tok_Int (int_of_string (String.sub matched 1 ((String.length matched) - 2)))])) else
    ...
    if Str.string_match greaterRegex input 0 then 
        if Str.string_match greaterEqualRegex input 0 then tokenize_helper (Str.string_after input 2) (acc @ [Tok_GreaterEqual]) else
        tokenize_helper (Str.string_after input 1) (acc @ [Tok_Greater]) else
    if Str.string_match lessRegex input 0 then 
        ...
        tokenize_helper (Str.string_after input 1) (acc @ [Tok_Less]) else
    if Str.string_match orRegex input 0 then tokenize_helper (Str.string_after input 2) (acc @ [Tok_Or]) else
    if Str.string_match andRegex input 0 then tokenize_helper (Str.string_after input 3) (acc @ [Tok_And]) else
    ...
    if Str.string_match boolofboolRegex input 0 then let matched = check_if_ID input (Str.matched_string input) in
        ...
        tokenize_helper(Str.string_after input (String.length matched)) (acc @ [Tok_ID matched]) else 
    if Str.string_match stringofstringRegex input 0 then let matched = trim_quotes (Str.matched_string input) true in
    ...
    raise (InvalidInputException "Unrecognized token")
